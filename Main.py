...
def plot_results(day, y_true, Y_pred_test):
    ...
    x = pd.date_range(d, periods=323, freq='5min')
    fig = plt.figure()
...
if __name__ == "__main__":
    ...
    FCN_BDLSTM_MSE_RMSE, history_2__LSTM = AttBDLSTMS_FCN(X_train_scale, Y_train_scale, epochs=model_epoch)

    FCN_BDLSTM_MSE_RMSE.save('TL-6_2-AttBDLSTMS_FCN20_____' + str(len(history_2__LSTM.losses)) + 'ep' + '_tl' + str(time_lag) + '.h5')
   ...
   # print(Y_test[0:5])
    ...
    plt.plot(aa, y_true[:15], marker='.', label="actual")
    plt.plot(aa, Y_pred_test[:15], 'r', label="prediction")
